...
/**
 * A geração do PDF do contrato, sem Nest.
 *
 * Mora fora do service — que é só o invólucro injetável — porque o **gerador
 * da loja de demonstração** (`packages/db/prisma/demo.ts`) precisa produzir um
 * contrato com o hash correto. Copiar estas quarenta linhas lá seria copiar a
 * definição de "qual PDF este contrato é": bastaria uma margem mudar de um
 * lado para o download do contrato da demo passar a responder 409 dizendo que
 * o documento foi alterado. Uma função pura, importada pelos dois, não tem
 * como divergir.
 */
 ...
 * hash.
 */
export async function gerarPdfDoContrato(
  blocos: Bloco[],
