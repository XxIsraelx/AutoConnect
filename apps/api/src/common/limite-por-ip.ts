/**
 * Teto de tentativas por chave, numa janela deslizante em memória de processo.
 *
 * Proporcional ao problema: o que se quer evitar é o script bobo que despeja
 * cem leads falsos num veículo ou cem lojas de mentira no cadastro, não um
 * ataque coordenado. Por isso **não** há CAPTCHA (custo de conversão alto para
 * quem está comprando carro, e alto demais para quem está avaliando o produto)
 * e **não** há Redis — o projeto não usa Redis, e trazer uma dependência de
 * infraestrutura para isto seria caro demais pelo que entrega.
 *
 ...
 *
 * Dois consumidores hoje, e uma implementação só:
 *
 * - **formulário público de lead** (`modules/leads/limite-por-ip.ts`) — 5
 *   envios por IP+loja+veículo em 10 minutos, somado a telefone brasileiro
 *   validado e honeypot;
 * - **cadastro de concessionária** (`modules/auth/limite-de-cadastro.ts`) —
 *   desde que a porta deixou de exigir convite, somado ao dígito verificador do
 *   CNPJ e ao CNPJ único por loja.
 *
 * Mora em `common/` justamente porque uma segunda cópia dessa janela seria uma
 * segunda chance de errar a poda do mapa, que é a parte que derruba o processo.
 */
