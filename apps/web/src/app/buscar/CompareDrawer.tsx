...
export default function CompareDrawer({
  ...
  const rows: { label: string; render: (v: PublicVehicle, i: number) => React.ReactNode }[] = [
    { label: 'Preço', render: (v, i) => (
      <span className={`font-extrabold ${i === bestPrice ? 'text-emerald-400' : 'text-white'}`}>
        {brl(v.promoPrice ?? v.price)}
      </span>
    )},
    { label: 'Ano', render: (v, i) => (
      <span className={i === bestYear ? 'text-emerald-400 font-bold' : ''}>{v.yearModel}</span>
    )},
    { label: 'KM', render: (v, i) => (
      <span className={i === bestKm ? 'text-emerald-400 font-bold' : ''}>
  ...
  return (
    <>
      ...
      {/* Drawer */}
      <aside
        className={`fixed top-0 right-0 z-[2001] h-full w-full max-w-2xl bg-[#0f172a] border-l border-white/[.08]
                    shadow-2xl transition-transform duration-300 flex flex-col
          ${open ? 'translate-x-0' : 'translate-x-full'}`}
      >
        {/* Header */}
        <div className="flex items-center justify-between px-5 h-14 border-b border-white/[.08] shrink-0">
          <div className="flex items-center gap-2">
            <GitCompare size={16} className="text-blue-400" />
            <h2 className="text-sm font-bold text-white">Comparar veículos</h2>
            <span className="text-xs text-slate-500">({vehicles.length})</span>
          </div>
          <button onClick={onClose} className="p-1.5 rounded-lg text-slate-400 hover:text-white hover:bg-white/[.06] transition">
            <X size={18} />
          </button>
        </div>
        ...
        {vehicles.length === 0 ? (
          ...
          <div className="flex-1 overflow-auto">
            {/* Cabeçalho com fotos */}
            <div className="grid sticky top-0 bg-[#0f172a] z-10 border-b border-white/[.06]"
                 style={{ gridTemplateColumns: `90px repeat(${vehicles.length}, minmax(0,1fr))` }}>
              <div className="p-3" />
              {vehicles.map((v) => {
                const cover = v.images[0]?.url;
                return (
                  <div key={v.id} className="p-3 border-l border-white/[.06] relative">
                    <button
                    ...
                    <div className="aspect-[4/3] rounded-lg overflow-hidden bg-white/5 mb-2 flex items-center justify-center">
                      {cover
                    ...
                    <p className="text-xs font-bold text-white truncate">{v.model.name}</p>
            ...
            {rows.map((row, ri) => (
              <div key={row.label}
                   className={`grid ${ri % 2 ? 'bg-white/[.015]' : ''}`}
                   style={{ gridTemplateColumns: `90px repeat(${vehicles.length}, minmax(0,1fr))` }}>
                <div className="p-3 text-[11px] font-semibold text-slate-500 uppercase tracking-wider">
                  {row.label}
                </div>
                {vehicles.map((v, i) => (
                  <div key={v.id} className="p-3 text-sm text-slate-200 border-l border-white/[.06] truncate">
                    {row.render(v, i)}
                  </div>
                ))}
              </div>
