    ...
    <button onClick={onClick}
      className={`relative w-9 h-9 rounded-xl flex items-center justify-center transition-all border
        ${active ? 'bg-blue-600 text-white border-blue-500'
                 : 'bg-white/[.05] text-slate-400 border-white/[.08] hover:text-white hover:bg-white/[.08]'}`}>
      <Icon size={16} />
      {count !== undefined && count > 0 && (
        <span className="absolute -top-1.5 -right-1.5 min-w-[16px] h-[16px] px-1 bg-rose-500 text-white text-[10px] font-bold rounded-full flex items-center justify-center border-2 border-[#0f172a]">
          {count > 9 ? '9+' : count}
        </span>
      )}
      {dot && count === undefined && (
            ...
            {favs.length === 0
              ...
              : favs.slice(0, 6).map((f) => (
                <Link key={f.vehicle.id} href={`/catalogo/${f.vehicle.tenantId}`} onClick={() => setPanel(null)}
                  className="flex items-center gap-2.5 px-3 py-2 hover:bg-white/[.04] transition">
                  <Thumb v={f.vehicle} />
                  <div className="min-w-0 flex-1">
                    <p className="text-xs font-bold text-white truncate">{f.vehicle.brand.name} {f.vehicle.model.name}</p>
              ...
              <>
                {reached.map((a) => (
                  <Link key={`al-${a.vehicle.id}`} href="/perfil" onClick={() => setPanel(null)}
                    className="flex items-start gap-2.5 px-3 py-2.5 hover:bg-white/[.04] transition">
                    ...
                    <div className="min-w-0">
                      <p className="text-xs text-white"><b>{a.vehicle.brand.name} {a.vehicle.model.name}</b> atingiu seu alvo</p>
                ...
                {newSearch.map((s) => (
                  <Link key={`se-${s.id}`} href="/perfil" onClick={() => setPanel(null)}
                    ...
                    <div className="min-w-0">
                      <p className="text-xs text-white"><b>{s.newCount}</b> novo{s.newCount !== 1 ? 's' : ''} em <b>{s.name}</b></p>
                ...
                {convs.filter((c) => (c.unreadCountCustomer ?? 0) > 0).map((c) => (
                  <button key={`co-${c.id}`} onClick={() => { setActiveConv(c); setPanel(null); }}
                    className="w-full flex items-start gap-2.5 px-3 py-2.5 hover:bg-white/[.04] transition text-left">
                    ...
                    <div className="min-w-0">
                      <p className="text-xs text-white"><b>{c.tenant.tradeName}</b> te respondeu</p>
              ...
              : convs.map((c) => {
                ...
                return (
                  <button key={c.id} onClick={() => { setActiveConv(c); setPanel(null); }}
                    className="w-full flex items-center gap-2.5 px-3 py-2.5 hover:bg-white/[.04] transition text-left">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-xs font-bold text-white truncate">{c.tenant.tradeName}</p>
...
function Dropdown({ title, children, footerHref, footerLabel }: {
  ...
  return (
    <div className="absolute right-0 top-full mt-2 w-72 bg-[#1e293b] border border-white/[.1] rounded-2xl shadow-2xl overflow-hidden z-[1001]">
      <div className="px-3 py-2.5 border-b border-white/[.06]">
        <p className="text-xs font-bold text-white">{title}</p>
      </div>
      <div className="max-h-[60vh] overflow-y-auto py-1">{children}</div>
      {footerHref && (
        <Link href={footerHref} className="block text-center text-[11px] font-semibold text-blue-400 hover:text-blue-300 py-2.5 border-t border-white/[.06] hover:bg-white/[.03] transition">
          {footerLabel}
        </Link>
      )}
    </div>
...
function Thumb({ v }: { v: SmallVehicle }) {
  return (
    <div className="w-11 h-9 rounded-lg bg-white/[.06] overflow-hidden shrink-0 flex items-center justify-center">
      {v.images[0]
        // eslint-disable-next-line @next/next/no-img-element
...
function Empty({ text }: { text: string }) {
  return <p className="text-xs text-slate-600 text-center py-6 px-3">{text}</p>;
}
