...
export default function CustomerSignupPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setLoading(true);
    try {
      const data = await api<{ accessToken: string; user: AuthUser }>('/auth/signup-customer', {
        method: 'POST',
    ...
    <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-8 shadow-sm">
      <div className="mb-6">
        ...
        <p className="text-sm text-slate-500 mt-1">
          Salve favoritos, receba alertas e negocie direto com concessionárias
        </p>
      </div>
      ...
      <p className="mt-5 text-center text-sm text-slate-500">
        Já tem conta?{' '}
        <Link href="/entrar" className="text-brand-accent hover:underline font-medium">
          Entrar
        </Link>
      </p>
    </div>
