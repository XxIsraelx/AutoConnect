...
export default function ChatDrawer({
  ...
  return createPortal(
    <>
      ...
      <aside className="fixed top-0 right-0 z-[2001] h-full w-full max-w-md sup-base border-l borda shadow-2xl flex flex-col txt-forte">
        <div className="flex items-center gap-3 px-4 h-14 border-b borda shrink-0">
          ...
          <button onClick={onClose} className="p-1.5 rounded-lg txt-fraco hover:txt-forte hover:sup-fraca"><X size={18} /></button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-2.5">
          {loading ? (
            ...
            return (
              <div key={m.id} className={`flex ${mine ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-[78%] rounded-2xl px-3.5 py-2 text-sm ${mine ? 'bg-blue-600 text-white rounded-tr-sm' : 'sup-card txt-medio rounded-tl-sm'}`}>
        ...
        <div className="p-3 border-t borda flex items-center gap-2 shrink-0">
          <input value={text} onChange={(e) => setText(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && send()}
            placeholder="Escreva uma mensagem…"
            className="flex-1 rounded-xl sup-card border borda px-3 py-2.5 text-sm txt-forte outline-none focus:border-blue-500" />
